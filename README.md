...
Около 2х лет назад я познакомился с так называемой **«Новой хронологией»** и, как и многие люди знакомые со школьным курсом Всемирной истории, был удивлен выводами данной теории.
...
Мое особое внимание привлекла программа [**horos.exe**](https://chronologia.org/horos.html ), которая служит основным источником проверки точности датировок "**Новой хронологии**" при изучении древних гороскопов (например в книге ["НОВАЯ ХРОНОЛОГИЯ ЕГИПТА"](https://chronologia.org/bibliography.html)).

Ниже представлено описание программы **horos.exe**: файл [README.TXT](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/HOROS/README.TXT) (расположенный в архиве с программой **horos.exe**). Оригинальная кодировка CP 866.
...
После ознакомления с описанием программы я решил проверить ее работоспособность и точность получаемых с ее помощью результатов. В качестве примера я взял выдержку из книги [**"Новая хронология Египта"**](https://chronologia.org/nx_egypt/index.html) (ссылка на которую указана на странице с программой), а именно разбор [**Дендерского Зодиака**](https://ru.wikipedia.org/wiki/%D0%94%D0%B5%D0%BD%D0%B4%D0%B5%D1%80%D1%81%D0%BA%D0%B8%D0%B9_%D0%B7%D0%BE%D0%B4%D0%B8%D0%B0%D0%BA).
...
2) Издание ["Новая хронология Египта"](https://chronologia.org/nx_egypt/index.html) Г.В.Носовский и А.Т.Фоменко. Исследования 2000-2001 годов. **Пункт 6.4.6** (см. ниже):
...
К сожалению, автор не приводит исходных текстов программы (а также параметры компиляции), что позволяет судить о **horos.exe** как о черном ящике, процессы внутри которого по тем или иным причинам скрыты от читателей **"Новой хронологии"**.

В файле [**README.TXT**](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/HOROS/README.TXT) содержатся лишь упоминания следующих работ:

   - [**Numerical expressions for precession formulae and mean elements for the Moon and the planets**](https://adsabs.harvard.edu/full/1994A%26A...282..663S)   [(**копия**)](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/1994A%2BA___282__663S.pdf)
   - [**ELP 2000-85 - A semi-analytical lunar ephemeris adequate for historical times**](https://adsabs.harvard.edu/full/1988A%26A...190..342C)   [(**копия**)](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/1988A%2BA___190__342C.pdf)

Читателю предлагается поверить наслово, что программа **horos.exe** имеет хоть какое-то отношение к указанным выше статьям и расчету датировок.

На этом исследование можно завершить, но я все таки решил проверить работу **horos.exe**, используя различные входные данные и сравнить степень правдоподобности результатов как с выводами самих авторов **"Новой Хронологии"**, так и с независимыми источниками.
...
Далее из описания програмы следует, что:
...
Стоит также отметить, что в расчетах присутсвует поправка на високосные года, но при этом отсутсвует поправка на смену календарей (лунный, римский, юлианский, грегорианский), а также на географическое местоположение "обозревателя" звездного неба (южное/северное полушария).

Без учета смены календарей и местоположения наблюдателя невозможно точно определить какие именно даты сопоставляются с общепризнанными и о каких цивилизациях/культурах идет речь.
...
Для быстрой проверки результатов работы программы **horos.exe** мной был написан скрипт [**horos-parser.py**](https://github.com/ponwork/chronologia.org-horos/blob/main/horos-parser.py) который:
   ...
   - в качестве аргумента принимает файл-результат (если не указан, берет файл **OTVET.TXT** в директории со скриптом);
   - считает общее количество решений;
   - считает количество решений с неизменным порядком планет;
   - находит все строки с указанием на то, что порядок планет не менялся (критерий, по которому отобран ответ в книге **"Новая хронология Египта"**);
   - на основе предыдущего результата создает словарь с номерами строк оригинального файла и соотвествующих им значениий "отклонения от лучших точек"
   - выдает в качестве решения ответ с минимальным показателем "отклонения от лучших точек" (также критерий, указанный автором книги **"Новая хронология Египта"**)

Итог работы скрипта [**horos-parser.py**](https://github.com/ponwork/chronologia.org-horos/blob/main/horos-parser.py) над файлом результата программы **horos.exe** с данными, взятыми из книги **"Новая хронология Египта"** (файл ["**otvet-fn.txt**"](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/otvet-fn.txt)) выдает следующее:

![result-fn](https://github.com/ponwork/chronologia.org-horos/blob/main/resources/result-fn.jpg)

Как видно, минимальное "отклонение от лучших точек" дает дата **"11 Марта 1422"** (**4.4 deg**) года , а не **"20 Марта 1185"** (**8.5 deg**) года, как указано в книге.
...
Почему был выбран именно этот результат, а не более точный (с отклонением **4.4**) -- ответ остается на совести автора книги.

**Примечание:** *код скрипта по нахождению лучшего решения был написан с целью получения скорейшего результата. Если вы нашли неточности или хотите его улучшить, просьба сообщить об этом через issues/PR.*
...
и произвести расчет даты рождения **Фоменко А.Т.** используя геоцентрическую карту звездного неба на **13 марта 1945 г.** и программу **horos.exe**.
...
Примечание: положение Луны явно не указано, поэтому берем его равным 0-12 и 999, как указано в описании к программе.
...
Как видно, программа ни одного решения не предлагает.

## ВЫВОДЫ:

1) Ввиду отсутсвия исходного кода программы и параметров ее компиляции утверждать о соответсвии ее результатов с чем-либо, включая реальные исторические события, не представляется возможным;
2) При вводе данных в программу не предусмотрена возможность учета местоположения "наблюдателя", а также отсуствует поправка на смену календарей в период с 500 года д.н.э. по 2000 год н.э. Без учета смены календарей и местоположения наблюдателя невозможно точно определить какие именно даты сопоставляются с общепризнанными и о каких цивилизациях/культурах идет речь.
3) Результат работы программы противоречит выводам, описанным в книге (при условии выбора результата с наименьшим отклонением, при отсутсвии изменения порядка планет). См. выше **"Проверка. Этап 3"**;
4) При расчете результата на заведомо известную дату программа не дает решений. См. выше **"Проверка. Этап 4"**.



Для устранения описанных выше недостатков и несоответсвий автору программы **horos.exe** предлагается предоставить:
   ...
   - любые другие данные, устраняющие недостатки и несоотвествия в работе **horos.exe**.



# Без предоставления указанных выше данных и их последующей проверки у меня есть все основания заявить, что выводы, изложенные в "Новой Хронологии" Г.В. Носовского и А.Т. Фоменко и основанные на результатах работы программы **horos.exe**, являются результатом манипуляций и подтасовок и никакого отношения к реальным событиям Мировой истории не имееют.
